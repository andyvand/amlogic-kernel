...
{	
	...
	if(!aml_chip->fbbt_protect){
		if(((blk == shipped_bbtinfo->valid_blk_addr) )&&(shipped_bbtinfo->valid_blk_addr >= start_blk)&&(!(aml_chip->protect & 1))){
			aml_nand_msg("protect fbbt at blk %d",blk);
			ret = -1;
		}else if(((blk == retry_info->info_save_blk)&&(retry_info->info_save_blk >= start_blk)&&(flash->new_type)&&(flash->new_type < 10))){
			aml_nand_msg("protect hynix retry info at blk %d",blk);
			ret = 0;
		}else if((blk == nand_key->valid_blk_addr)&&(nand_key->valid_blk_addr >= start_blk)&&(!(aml_chip->protect & (1<<1)))){
			aml_nand_msg("protect nand_key info at blk %d",blk);
			ret = 0;
		}else if((blk == nand_secure->valid_blk_addr)&&(nand_secure->valid_blk_addr >= start_blk)&&(!(aml_chip->protect & (1<<2)))){
